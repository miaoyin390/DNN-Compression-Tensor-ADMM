...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        if not config.load_compressed_model:
            ...
            # self.word_embeddings = TTEmbedding(input_tt_shape=[32, 31, 31], output_tt_shape=[24, 32],
...
class TTBertSelfAttention(nn.Module):  # not intilized dense if use compresstion
    def __init__(self, config, layer_id=None):
        ...
        self.query = TTLinear(in_features=config.hidden_size, out_features=config.hidden_size,
                              in_shapes=[24, 32], out_shapes=[768],
                              ranks=[1, 18, 18, 1])
...
class BertIntermediate(nn.Module):  # not intilized dense if use compresstion
    def __init__(self, config, intermediate_size=-1, layer_id=None):
        ...
        self.dense = TTLinear(in_features=config.hidden_size, out_features=intermediate_size,
                              in_shapes=[config.hidden_size], out_shapes=[64, 48],
                              ranks=[1, 18, 18, 1])
...
class BertOutput(nn.Module):
    def __init__(self, config, intermediate_size=-1, layer_id=None):
        ...
        self.dense = TTLinear(in_features=intermediate_size, out_features=config.hidden_size,
                              in_shapes=[48, 64], out_shapes=[config.hidden_size],
                              ranks=[1, 17, 17, 1])
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
