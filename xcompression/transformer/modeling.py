...
class TuckerWeights(nn.Module):
    def __init__(self, config):  #
        ...
        self.factor_left = nn.Parameter(torch.zeros(config.hidden_size, config.rank_condim),
                                        requires_grad=config.requires_grad)
        self.factor_right = nn.Parameter(torch.zeros(config.rank_dim, config.hidden_size),
                                         requires_grad=config.requires_grad)
        self.factor_layer = nn.Parameter(torch.zeros(config.num_hidden_layers * self.blocks_num, config.rank_layer),
                                         requires_grad=config.requires_grad)
    ...
    def forward(self, keep_gloabl_weights=True):  # change the order
        if not keep_gloabl_weights:
            return torch.einsum("ikl,xi,yk,lz->xyz",
                                [self.core, self.factor_layer, self.factor_left, self.factor_right]).view(-1, 12,
                                                                                                          self.hidden_size,
        ...
        # need ffn or not
        return core.view(-1, self.blocks_num, core.size()[-1],
                         core.size()[-2]), self.bias, self.factor_left, self.factor_right
    ...
    def ffn_inner_transform(self, hidden_states, layer):
        ...
        right_temp = weight[-8:-4].reshape(-1, weight.size()[-1]).transpose(-1, -2).view(rank, 4,
                                                                                         rank) @ self.factor_left.transpose(
            -1, -2)
    ...
    def ffn_output_transform(self, hidden_states, layer):
        ...
        hidden_states = hidden_states.view(hidden_states.size()[0], hidden_states.size()[1],
                                           -1) @ right_temp + bias  # b*4*d * 4d*D
    ...
    def get_weights_from_encoder(self, config, encoder):
        ...
        for l in range(config.num_hidden_layers):
            ...
            if "ffn" in self.ops:
                ...
                W_output = encoder.layer[l].output.dense.weight.transpose(0, 1).view(-1, config.hidden_size,
                                                                                     config.hidden_size)
    ...
    def raw_initialize(self, config, encoder):
        ...
        print("again average square error during compression： {}".format(
            ((y - weights) ** 2).mean().cpu().detach().numpy()))
...
class TuckerWeights_Plus(TuckerWeights):
    def __init__(self, config):  #
        ...
        self.exp_left = nn.Parameter(
            torch.rand(config.num_hidden_layers * self.blocks_num, config.hidden_size, config.rank_left_expdim) / 100,
            requires_grad=config.requires_grad)
        self.exp_right = nn.Parameter(
            torch.rand(config.num_hidden_layers * self.blocks_num, config.rank_right_expdim, config.hidden_size) / 100,
            requires_grad=config.requires_grad)
        self.exp_core11 = self.core
        self.exp_core12 = nn.Parameter(
            torch.rand(config.num_hidden_layers * self.blocks_num, config.rank_condim, config.rank_right_expdim) / 100,
            requires_grad=config.requires_grad)
        self.exp_core21 = nn.Parameter(
            torch.rand(config.num_hidden_layers * self.blocks_num, config.rank_left_expdim, config.rank_dim) / 100,
            requires_grad=config.requires_grad)
        self.exp_core22 = nn.Parameter(torch.rand(config.num_hidden_layers * self.blocks_num, config.rank_left_expdim,
                                                  config.rank_right_expdim) / 100,
                                       requires_grad=config.requires_grad)
    ...
    def qkv_transform(self, hidden_states, layer):
        ...
        mixed_query_layer = hidden_states @ (torch.cat((self.factor_left, u2[0]), 1) @ weight[0]).tanh() @ torch.cat(
            (self.factor_right, v2[0]), 0) + bias[0]  #
        mixed_key_layer = hidden_states @ (torch.cat((self.factor_left, u2[1]), 1) @ weight[1]).tanh() @ torch.cat(
            (self.factor_right, v2[1]), 0) + bias[1]  #
        mixed_value_layer = hidden_states @ (torch.cat((self.factor_left, u2[2]), 1) @ weight[2]).tanh() @ torch.cat(
            (self.factor_right, v2[2]), 0) + bias[2]  #
    ...
    def output_transform(self, hidden_states, layer):
        ...
        hidden_states = hidden_states @ (torch.cat((self.factor_left, u2[3]), 1) @ weight[3]).tanh() @ torch.cat(
            (self.factor_right, v2[3]), 0) + bias[3]  #
        return hidden_states
    ...
    def init_weights(self, config, encoder):
        ...
        print("again average l1 error during compression： {}".format(
            (y11 + y12 + y21 + y22 - weights).abs().mean().cpu().detach().numpy()))
