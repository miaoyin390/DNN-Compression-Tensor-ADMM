...
from transformer.SVDLinear import SVDLinear
import time
import copy
import json
import logging
...
# Transformer
class SVDBertSelfAttention(nn.Module):  # not intilized dense if use compresstion
    def __init__(self, config, layer_id=None):
        super(SVDBertSelfAttention, self).__init__()
        ...
        self.all_head_size = config.rank
        self.head_rank = config.rank // self.num_attention_heads
    ...
    def transpose_for_scores(self, x):
        new_x_shape = x.size()[
                      :-1] + (self.num_attention_heads, self.head_rank)
...
class BertAttention(nn.Module):
    def __init__(self, config, layer_id=None):
        ...
        self.layer_id = layer_id
        self.self = SVDBertSelfAttention(config, layer_id=layer_id)
        self.output = SVDBertSelfOutput(config, layer_id=layer_id)
...
class SVDBertSelfOutput(nn.Module):
    def __init__(self, config, layer_id=None):
        super(SVDBertSelfOutput, self).__init__()
        self.layer_id = layer_id
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        # if not config.load_compressed_model or (config.load_compressed_model and "san" not in config.ops):
        self.dense = nn.Linear(config.rank, config.hidden_size)
...
class SVDBertIntermediate(nn.Module):
    def __init__(self, config, intermediate_size=-1, layer_id=None):
        super(SVDBertIntermediate, self).__init__()
        self.layer_id = layer_id
        if intermediate_size < 0:
            intermediate_size = config.intermediate_size
        # if not config.load_compressed_model or (config.load_compressed_model and "ffn" not in config.ops):
        self.dense = SVDLinear(in_features=config.hidden_size, out_features=intermediate_size, rank=config.rank)
...
class SVDBertOutput(nn.Module):
    def __init__(self, config, intermediate_size=-1, layer_id=None):
        super(SVDBertOutput, self).__init__()
        self.layer_id = layer_id
        if intermediate_size < 0:
            intermediate_size = config.intermediate_size
        # if not config.load_compressed_model or (config.load_compressed_model and "ffn" not in config.ops):
        # self.dense = nn.Linear(intermediate_size, config.hidden_size)
        self.dense = SVDLinear(in_features=intermediate_size, out_features=config.hidden_size, rank=config.rank)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertLayer(nn.Module):
    def __init__(self, config, layer_id=None):
        ...
        self.intermediate = SVDBertIntermediate(config, layer_id=layer_id)
        self.output = SVDBertOutput(config, layer_id=layer_id)
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        encoded_layers, layer_atts = self.encoder(embedding_output,
                                                  extended_attention_mask, self.ops)
