...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        if not config.load_compressed_model:
            ...
            # self.word_embeddings = TTEmbedding(input_tt_shape=[32, 31, 31], output_tt_shape=[32, 24],
...
class TTBertSelfAttention(nn.Module):  # not intilized dense if use compresstion
    def __init__(self, config, layer_id=None):
        ...
        self.query = TTMLinear(input_tt_shape=[32, 24], output_tt_shape=[32, 24],
                             tt_ranks=[1, 7, 1])
        self.key = TTMLinear(input_tt_shape=[32, 24], output_tt_shape=[32, 24],
                             tt_ranks=[1, 7, 1])
        self.value = TTMLinear(input_tt_shape=[32, 24], output_tt_shape=[32, 24],
                             tt_ranks=[1, 7, 1])
...
class TTBertSelfOutput(nn.Module):
    def __init__(self, config, layer_id=None):
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        # if not config.load_compressed_model or (config.load_compressed_model and "san" not in config.ops):
        self.dense = TTMLinear(input_tt_shape=[32, 24], output_tt_shape=[32, 24],
                             tt_ranks=[1, 7, 1])
...
class TTBertIntermediate(nn.Module):
    def __init__(self, config, intermediate_size=-1, layer_id=None):
        ...
        self.dense = TTMLinear(input_tt_shape=[32, 24], output_tt_shape=[64, 48],
                             tt_ranks=[1, 13, 1])
...
class TTBertOutput(nn.Module):
    def __init__(self, config, intermediate_size=-1, layer_id=None):
        ...
        self.dense = TTMLinear(input_tt_shape=[64, 48], output_tt_shape=[32, 24],
                             tt_ranks=[1, 12, 1])
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
