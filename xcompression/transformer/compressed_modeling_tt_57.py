...
class TTBertSelfAttention(nn.Module):  # not intilized dense if use compresstion
    def __init__(self, config, layer_id=None):
        ...
        self.query = TTLinear(in_features=config.hidden_size, out_features=config.hidden_size,
                              in_shapes=[32, 24], out_shapes=[32, 24],
                              compression_ratio=57)
        self.key = TTLinear(in_features=config.hidden_size, out_features=config.hidden_size,
                            in_shapes=[32, 24], out_shapes=[32, 24],
                            compression_ratio=57)
        self.value = TTLinear(in_features=config.hidden_size, out_features=config.hidden_size,
                              in_shapes=[32, 24], out_shapes=[32, 24],
                              compression_ratio=57)
...
class TTBertSelfOutput(nn.Module):
    def __init__(self, config, layer_id=None):
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        # if not config.load_compressed_model or (config.load_compressed_model and "san" not in config.ops):
        self.dense = TTLinear(in_features=config.hidden_size, out_features=config.hidden_size,
                              in_shapes=[32, 24], out_shapes=[32, 24],
                              compression_ratio=57)
...
class TTBertIntermediate(nn.Module):
    def __init__(self, config, intermediate_size=-1, layer_id=None):
        ...
        self.dense = TTLinear(in_features=config.hidden_size, out_features=intermediate_size,
...
class TTBertOutput(nn.Module):
    def __init__(self, config, intermediate_size=-1, layer_id=None):
        ...
        self.dense = TTLinear(in_features=intermediate_size, out_features=config.hidden_size,
