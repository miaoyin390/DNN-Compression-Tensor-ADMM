...
from transformer.TTMLinear import TTMLinear
import copy
import json
import logging
import math
...
NORM = {'layer_norm': nn.LayerNorm}


class CompressedBertConfig(object):
    ...
    def __init__(self,
        ...
        if isinstance(vocab_size_or_config_json_file, str) or (sys.version_info[0] == 2
            ...
            self.rank = head_rank * num_attention_heads
        else:
    ...
    def from_dict(cls, json_object):
        ...
        config = CompressedBertConfig(vocab_size_or_config_json_file=-1)
...
def EmbeddingFactory(config):
    if "embedding_type" in config.__dict__ and config.embedding_type == "ket":
        ...
        word_embeddings = TTEmbedding(input_tt_shape=[13, 13, 13, 14], output_tt_shape=[8, 4, 4, 6],
                                      tt_ranks=[1] + [15] * 7 + [1])
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        if not config.load_compressed_model:
            ...
            # self.word_embeddings = TTEmbedding(input_tt_shape=[32, 31, 31], output_tt_shape=[12, 8, 8],
            #                                    tt_ranks=[1, 30, 30, 25, 25, 1])
        else:
        ...
        # self.position_embeddings = TTEmbedding(input_tt_shape=[8, 8, 8], output_tt_shape=[12, 8, 8],
        #                                        tt_ranks=[1, 6, 6, 6, 6, 6, 1])
...
# Transformer
class TTBertSelfAttention(nn.Module):  # not intilized dense if use compresstion
    def __init__(self, config, layer_id=None):
        super(TTBertSelfAttention, self).__init__()
        ...
        self.query = TTMLinear(input_tt_shape=[12, 8, 8], output_tt_shape=[12, 8, 8],
                             tt_ranks=[1, 10, 10, 1])
        self.key = TTMLinear(input_tt_shape=[12, 8, 8], output_tt_shape=[12, 8, 8],
                             tt_ranks=[1, 10, 10, 1])
        self.value = TTMLinear(input_tt_shape=[12, 8, 8], output_tt_shape=[12, 8, 8],
                             tt_ranks=[1, 10, 10, 1])
...
class BertAttention(nn.Module):
    def __init__(self, config, layer_id=None):
        ...
        self.layer_id = layer_id
        self.self = TTBertSelfAttention(config, layer_id=layer_id)
        self.output = TTBertSelfOutput(config, layer_id=layer_id)
...
class TTBertSelfOutput(nn.Module):
    def __init__(self, config, layer_id=None):
        super(TTBertSelfOutput, self).__init__()
        self.layer_id = layer_id
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        # if not config.load_compressed_model or (config.load_compressed_model and "san" not in config.ops):
        self.dense = TTMLinear(input_tt_shape=[12, 8, 8], output_tt_shape=[12, 8, 8],
                             tt_ranks=[1, 10, 10, 1])
...
class TTBertIntermediate(nn.Module):
    def __init__(self, config, intermediate_size=-1, layer_id=None):
        super(TTBertIntermediate, self).__init__()
        self.layer_id = layer_id
        if intermediate_size < 0:
            intermediate_size = config.intermediate_size
        # if not config.load_compressed_model or (config.load_compressed_model and "ffn" not in config.ops):
        self.dense = TTMLinear(input_tt_shape=[12, 8, 8], output_tt_shape=[16, 16, 12],
                             tt_ranks=[1, 18, 17, 1])
    ...
    def forward(self, hidden_states):  # since it was once transposed
...
class TTBertOutput(nn.Module):
    def __init__(self, config, intermediate_size=-1, layer_id=None):
        super(TTBertOutput, self).__init__()
        self.layer_id = layer_id
        if intermediate_size < 0:
            intermediate_size = config.intermediate_size
        # if not config.load_compressed_model or (config.load_compressed_model and "ffn" not in config.ops):
        # self.dense = nn.Linear(intermediate_size, config.hidden_size)
        self.dense = TTMLinear(input_tt_shape=[16, 16, 12], output_tt_shape=[12, 8, 8],
                             tt_ranks=[1, 18, 18, 1])
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertLayer(nn.Module):
    def __init__(self, config, layer_id=None):
        ...
        self.intermediate = TTBertIntermediate(config, layer_id=layer_id)
        self.output = TTBertOutput(config, layer_id=layer_id)
...
class BertPooler(nn.Module):
    def __init__(self, config, recurs=None):
        ...
        # self.dense = TTMLinear(in_features=config.hidden_size, out_features=config.hidden_size,
        #                       in_shapes=[12, 8, 8], out_shapes=[12, 8, 8],
        #                       ranks=[1, 4, 4, 4, 1])
...
class CompressedBertPreTrainedModel(nn.Module):
    """ An abstract class to handle weights initialization and
        a simple interface for dowloading and loading pretrained models.
    """

    def __init__(self, config, *inputs, **kwargs):
        super(CompressedBertPreTrainedModel, self).__init__()
        if not isinstance(config, CompressedBertConfig):
            raise ValueError(
    ...
    def from_scratch(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        config = CompressedBertConfig.from_json_file(resolved_config_file)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        config = CompressedBertConfig.from_json_file(config_file)
        logger.info("Model config {}".format(config))
        # Instantiate model.
...
class CompressedBertModel(CompressedBertPreTrainedModel):
...
class BertForPreTraining(CompressedBertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert = CompressedBertModel(config)
...
class BertForMaskedLM(CompressedBertPreTrainedModel):
...
class BertForNextSentencePrediction(CompressedBertPreTrainedModel):
...
class BertForSentencePairClassification(CompressedBertPreTrainedModel):
...
class CompressedBertForSequenceClassification(CompressedBertPreTrainedModel):
...
class BertForQuestionAnswering(CompressedBertPreTrainedModel):
    r"""
