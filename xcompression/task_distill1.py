...
def frozen_embedding(src_model):
    if hasattr(src_model, 'module'):
        model = src_model.module
    else:
        model = src_model
    ...
    model.bert.pooler.dense.weight.requires_grad = False
...
def train_and_evaluate(args, teacher_model, student_model, train_dataloader, tokenizer, label_list, device, output_mode,
    ...
    if not args.pred_distill:
        args.num_train_epochs = default_params_first[task_name]["num_train_epochs"] * 2
    else:
        args.num_train_epochs = default_params_second[task_name]["num_train_epochs"] * 2
    ...
    for epoch_ in trange(int(args.num_train_epochs), desc="Epoch"):
        beta_scale = max(math.cos(math.pi * 0.5 * epoch_ / args.num_train_epochs) * beta, 0.01)
        tr_loss = 0.
        tr_att_loss = 0.
        tr_rep_loss = 0.
        tr_cls_loss = 0.
        ...
        student_model.train()
        # frozen_embedding(student_model)
        ...
        for step, batch in enumerate(
            ...
            if args.finegrain:
                for i in align_ids:
                    tmp_loss = beta_scale * loss_mse(student_reps[i], teacher_reps[i])
                    rep_loss += tmp_loss
            ...
            if output_mode == "classification":
                if args.distill == 'soft':
                    ...
                    cls_loss = F.cross_entropy(student_logits, teacher_logits.argmax(dim=1))
            elif output_mode == "regression":
                loss_mse = MSELoss()
            ...
            if args.distill is not None:
                loss = 0.1 * loss + 0.5 * cls_loss + 0.5 * loss_ori
            else:
                loss = 0.5 * cls_loss + 0.5 * loss_ori
            ...
            if (global_step + 1) % args.eval_step == 0:
                ...
                if save_model:
                    ...
                    # Test mnli-mm
                    if args.distill is not None and task_name == "mnli":
                        task_name = "mnli-mm"
...
def main():
    ...
    # student_model = BertForSequenceClassification.from_pretrained(args.teacher_model, num_labels=num_labels)
    ...
    train_and_evaluate(args, teacher_model, student_model, train_dataloader, tokenizer, label_list, device,
                       output_mode, train_examples, num_labels, eval_examples, eval_labels, eval_dataloader,
                       eval_data)
